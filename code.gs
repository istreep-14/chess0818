...
const CHESS_USERNAME = 'your_username_here'; // Replace with your Chess.com username
...
function populateAllArchiveGames() {
  try {
    ...
    SpreadsheetApp.getUi().alert(`Error: ${error.message}`);
      ...
      sheet.getRange(2, 1, gameData.length, 6).setValues(gameData);
    }
    
    console.log(`Populated ${games.length} games for ${sheetName}`);
    
  } catch (error) {
    console.error(`Error populating games for ${sheetName}:`, error);
    throw error;
  }
}
...
function showHelp() {
  ...
  ui.alert('Instructions:\n\n' +
    ...
    '5. Individual sheets will be created for each archive (named like "a0825" for August 2025)\n' +
    '6. The "Last Updated" column shows when each archive was last populated\n\n' +
